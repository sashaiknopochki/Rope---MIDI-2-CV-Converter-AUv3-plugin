...
private let log = Logger(subsystem: "pro.gracescale.Rope.Rope---MIDI2CV", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<Rope___MIDI2CVMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try Rope___MIDI2CVAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? Rope___MIDI2CVAudioUnit else {
				log.error("Unable to create Rope___MIDI2CVAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(Rope___MIDI2CVParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = Rope___MIDI2CVMainView(parameterTree: observableParameterTree)
